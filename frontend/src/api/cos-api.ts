...
export interface MoldsByBatteryResponse {
  batteryName: string;
  keyword: string | null;
  count: number;
  molds: MoldDto[];
}
export const getMoldsByBattery = (batteryName: string) =>
  apiFetch<MoldsByBatteryResponse>(`/battery/molds-by-battery?batteryName=${encodeURIComponent(batteryName)}`);
...
export async function uploadMemoFile(submissionId: number, role: string, file: File): Promise<AttachmentDto> {
  const form = new FormData();
  form.append('role', role);
  form.append('file', file);
  const resp = await fetch(`${API_BASE}/formsubmission/${submissionId}/upload-memo`, {
    method: 'POST',
    body: form,
    // NOTE: Do NOT set Content-Type header — browser sets boundary automatically for FormData
  });
  if (!resp.ok) {
...
export async function deleteAttachment(attachmentId: number): Promise<void> {
  await apiFetch<void>(`/formsubmission/attachment/${attachmentId}`, { method: 'DELETE' });
}
...
// ══════════════════════════════════════════════════
// PRODUCTION — Running Battery Auto-Detect
// ══════════════════════════════════════════════════

export interface RunningBatteryDto {
  id: number;
  name: string;
  sourceItemNum: string | null;
  formId: number | null;
}

export interface RunningBatteriesResponse {
  date: string;
  lineId: number;
  shiftId: number;
  count: number;
  batteries: RunningBatteryDto[];
}

export const getRunningBatteries = (lineId: number, shiftId: number, date?: Date) => {
  let url = `/production/running-batteries?lineId=${lineId}&shiftId=${shiftId}`;
  if (date) {
    // Format as yyyy-MM-dd (local date, no timezone shift)
    const yyyy = date.getFullYear();
    const mm = String(date.getMonth() + 1).padStart(2, '0');
    const dd = String(date.getDate()).padStart(2, '0');
    url += `&date=${yyyy}-${mm}-${dd}`;
  }
  return apiFetch<RunningBatteriesResponse>(url);
};
