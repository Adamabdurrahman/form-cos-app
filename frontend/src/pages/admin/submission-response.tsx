...
export function SubmissionResponse() {
    ...
    // ===== LEADER MEMO TAB STATE =====
    const [leaderMemoTab, setLeaderMemoTab] = useState<'upload' | 'camera'>('upload');
    const [memoAttachments, setMemoAttachments] = useState<AttachmentDto[]>([]);
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                if ((submission as any).attachments) {
                    setMemoAttachments((submission as any).attachments);
                }
            } catch (err) {
    ...
    // ===== LEADER MEMO: FILE UPLOAD HANDLER =====
    const handleFileUpload = useCallback(async (files: FileList | null) => {
        if (!files || files.length === 0 || !id) return;
        setUploading(true);
        try {
            for (let i = 0; i < files.length; i++) {
                const result = await uploadMemoFile(Number(id), 'leader', files[i]);
                setMemoAttachments(prev => [...prev, result]);
            }
            notify('File berhasil diupload!', 'success', 2000);
        } catch (err) {
            console.error('Upload error:', err);
            notify('Gagal upload file.', 'error', 3000);
        } finally {
            setUploading(false);
    ...
    // ===== LEADER MEMO: DELETE ATTACHMENT HANDLER =====
    const handleDeleteAttachment = useCallback(async (attId: number) => {
        try {
            await deleteAttachment(attId);
            setMemoAttachments(prev => prev.filter(a => a.id !== attId));
            notify('File dihapus.', 'info', 2000);
        } catch (err) {
            console.error('Delete attachment error:', err);
            notify('Gagal menghapus file.', 'error', 3000);
        }
    }, []);

    // ===== LEADER MEMO: CAMERA HANDLERS =====
    const startCamera = useCallback(async () => {
        try {
            ...
            notify('Tidak bisa mengakses kamera. Pastikan izin kamera diberikan.', 'error', 4000);
        }
    }, []);
    ...
    const capturePhoto = useCallback(async () => {
        ...
        setUploading(true);
        try {
            const result = await uploadMemoFile(Number(id), 'leader', file);
            setMemoAttachments(prev => [...prev, result]);
            notify('Foto berhasil disimpan!', 'success', 2000);
            stopCamera();
        } catch (err) {
            console.error('Photo upload error:', err);
            notify('Gagal menyimpan foto.', 'error', 3000);
        } finally {
            setUploading(false);
        }
    }, [id, stopCamera]);
    ...
    const handleApprove = useCallback(async () => {
        ...
        // Leader: require at least memo text OR uploaded file when NG
        if (currentApprovalRole === 'leader' && hasNg && !approvalMemo.trim() && memoAttachments.length === 0) {
            notify('Memo atau file wajib diisi karena terdapat item NG.', 'warning', 3000);
            return;
        }
        try {
            ...
            if (currentApprovalRole === 'kasubsie') { setKasubsieSignedAt(now); }
            if (currentApprovalRole === 'kasie') { setKasieSignedAt(now); }
    ...
    }, [currentApprovalRole, approvalSig, approvalMemo, hasNg, id, memoAttachments, stopCamera]);
                ...
                <div className="approval-workflow-section no-print">
                    ...
                    {currentApprovalRole && (
                        <div className="approve-action-section">
                            ...
                            {/* ===== LEADER MEMO: TABBED INTERFACE ===== */}
                            {currentApprovalRole === 'leader' && (
                                <div className="leader-memo-section">
                                    <label className="approve-label">
                                        Memo / Bukti {hasNg ? <span className="required-mark">*wajib (ada NG)</span> : <span className="optional-mark">(opsional)</span>}
                                    </label>

                                    {/* Tab Buttons */}
                                    <div className="memo-tabs">
                                        <button
                                            className={`memo-tab ${leaderMemoTab === 'upload' ? 'active' : ''}`}
                                            onClick={() => { setLeaderMemoTab('upload'); stopCamera(); }}
                                            type="button"
                                        >
                                            <span className="tab-icon">📁</span> Upload File
                                        </button>
                                        <button
                                            className={`memo-tab ${leaderMemoTab === 'camera' ? 'active' : ''}`}
                                            onClick={() => setLeaderMemoTab('camera')}
                                            type="button"
                                        >
                                            <span className="tab-icon">📷</span> Ambil Foto
                                        </button>
                                    </div>

                                    {/* Tab Content */}
                                    <div className="memo-tab-content">
                                        {leaderMemoTab === 'upload' && (
                                            <div className="memo-upload-tab">
                                                ...
                                                >
                                                    ...
                                                    <div className="upload-text">
                                                        ...
                                                        <span>PDF, gambar, atau dokumen lainnya</span>
                                                    </div>
                                                </div>
                                                <input
                                                    ref={fileInputRef}
                                                ...
                                                />
                                                {uploading && <div className="upload-progress">Mengupload...</div>}
                                            </div>
                                        )}

                                        {leaderMemoTab === 'camera' && (
                                            <div className="memo-camera-tab">
                                                {!cameraActive ? (
                                                    <div className="camera-start">
                                                        <button className="camera-start-btn" onClick={startCamera} type="button">
                                                    ...
                                                    <div className="camera-preview">
                                                        ...
                                                        <div className="camera-controls">
                                                            <button className="capture-btn" onClick={capturePhoto} type="button" disabled={uploading}>
                                                                {uploading ? '⏳' : '📸'} {uploading ? 'Menyimpan...' : 'Ambil Foto'}
                                                            </button>
                                                            <button className="cancel-camera-btn" onClick={stopCamera} type="button">
                                                                ✕ Tutup Kamera
                                                            </button>
                                                        </div>
                                                    </div>
                                    ...
                                    {memoAttachments.length > 0 && (
                                        <div className="memo-attachments">
                                            <div className="attachments-title">File Terlampir ({memoAttachments.length})</div>
                                            <div className="attachments-grid">
                                                {memoAttachments.map(att => (
                                                    <div key={att.id} className="attachment-card">
                                                        {att.contentType?.startsWith('image/') ? (
                                                            <img src={att.filePath} alt={att.fileName} className="attachment-preview" />
                                                        ) : (
                                                            <div className="attachment-file-icon">📄</div>
                                                        )}
                                                        <div className="attachment-info">
                                                            <span className="attachment-name" title={att.fileName}>{att.fileName}</span>
                                                            <span className="attachment-date">{new Date(att.uploadedAt).toLocaleString('id-ID')}</span>
                                                        </div>
                                                        <button className="attachment-delete" onClick={() => handleDeleteAttachment(att.id)} type="button" title="Hapus">
                                                            🗑
                                                        </button>
                                                    </div>
                                                ))}
                                    ...
                                    {/* Optional text memo */}
                                    <div className="memo-text-section">
                                        <label className="approve-label">Catatan Tambahan <span className="optional-mark">(opsional)</span></label>
                                        <TextArea
                                            value={approvalMemo}
                                            onValueChanged={(e) => setApprovalMemo(e.value ?? '')}
                                            placeholder="Tulis catatan tambahan jika perlu..."
                                            height={70}
                                            stylingMode="outlined"
                                        />
                                    </div>
                            ...
                            {/* ===== KASUBSIE / KASIE: SIMPLE TEXT MEMO ===== */}
                            {currentApprovalRole !== 'leader' && (
                                <div className="approve-memo">
                                    <label className="approve-label">
                                        Catatan <span className="optional-mark">(opsional)</span>
                                    </label>
                                    <TextArea
                                        value={approvalMemo}
                                        onValueChanged={(e) => setApprovalMemo(e.value ?? '')}
                            ...
                            <div className="approve-btn-row">
                                <Button
                                    ...
                                    disabled={!approvalSig || (currentApprovalRole === 'leader' && hasNg && !approvalMemo.trim() && memoAttachments.length === 0)}
                                    width={220}
                                    height={40}
                                />
                            </div>
