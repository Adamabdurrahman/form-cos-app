...
export function CosValidation({ formCode = 'COS_VALIDATION' }: { formCode?: string }) {
    ...
    const [detectedBatteryNames, setDetectedBatteryNames] = useState<string[] | null>(null);
    const [moldList, setMoldList] = useState<MoldDto[]>([]);
    const [filteredMoldsBySlot, setFilteredMoldsBySlot] = useState<Record<number, MoldDto[]>>({});
    ...
    // ===== AUTO-SELECT OPERATOR (if logged-in user is operator) =====
    useEffect(() => {
        if (!loading && formDef && user?.role === 'operator' && !autoSelectDone.current) {
            autoSelectDone.current = true;
            // Only auto-set if not already set (e.g., by draft restore)
            if (!operatorEmpId) {
                setOperatorEmpId(user.empId);
                // Chain reaction: auto-fetch hierarchy
                getHierarchy(user.empId).then(h => {
                    setHierarchyIds({
                        leaderEmpId: h.leaderEmpId,
                        kasubsieEmpId: h.kasubsieEmpId,
                        kasieEmpId: h.kasieEmpId,
    ...
    }, [loading, formDef, user, operatorEmpId]);
    ...
    // ===== AUTO-DETECT RUNNING BATTERIES =====
    useEffect(() => {
        // Guard: both line & shift must be selected, and initial load must be done
        if (!lineId || !shiftId || !initialLoadDone.current || !formDef) return;

        let cancelled = false;

        async function detectBatteries() {
            try {
                const response = await getRunningBatteries(lineId!, shiftId!, tanggal);
                if (cancelled) return;

                if (response.batteries.length > 0) {
                    // Map API response to battery slots
                    const newSlots: BatterySlot[] = response.batteries.map((b) => ({
                        type: b.name,
                        mold: null,
                    }));
                    setBatterySlots(newSlots);
                    setSettings({});
                    // ── Strict filter: dropdown hanya tampilkan battery yang running ──
                    setDetectedBatteryNames(response.batteries.map(b => b.name));
                    console.log(
                        `[Auto-Detect] Found ${response.count} running battery type(s):`,
                        response.batteries.map(b => b.name)
                    );
                } else {
                    // No production data → reset to default empty slots (manual mode)
                    setBatterySlots(Array.from({ length: formDef!.slotCount }, () => ({ type: null, mold: null })));
                    setSettings({});
                    setDetectedBatteryNames(null); // ← back to manual mode (show all)
                    console.log('[Auto-Detect] No running batteries found — manual mode');
                }
            } catch (err) {
                console.error('[Auto-Detect] Failed to fetch running batteries:', err);
                // On error, don't break the form — leave slots as-is
            }
        }
    ...
    }, [lineId, shiftId, tanggal, formDef]);

    // ===== AUTO-FETCH FILTERED MOLDS PER BATTERY SLOT =====
    useEffect(() => {
        const newFiltered: Record<number, MoldDto[]> = {};
        let pending = 0;

        batterySlots.forEach((slot, idx) => {
            if (slot.type) {
                pending++;
                getMoldsByBattery(slot.type)
                    .then(resp => {
                        newFiltered[idx] = resp.molds;
                    })
                    .catch(err => {
                        console.error(`[Mold Filter] Slot ${idx}:`, err);
                        newFiltered[idx] = [];
                    })
                    .finally(() => {
                        pending--;
                        if (pending === 0) {
                            setFilteredMoldsBySlot(newFiltered);
                        }
                    });
            }
        });

        // If no slots have battery types, clear all filtered molds
        if (pending === 0) {
            setFilteredMoldsBySlot({});
        }
    }, [batterySlots]);
    ...
    const handleLineChange = useCallback((e: { value?: number | null }) => {
        ...
        // Reset downstream — BUT preserve operator if user IS the operator (auto-selected)
        const isAutoOperator = user?.role === 'operator';
        if (!isAutoOperator) {
    ...
    const getMoldsForSlot = useCallback((slotIdx: number) => {
        // If we have filtered molds for this slot, use those; otherwise fall back to full list
        const filtered = filteredMoldsBySlot[slotIdx];
        if (filtered) return filtered.map(m => m.code);
        return moldList.map(m => m.code);
    }, [moldList, filteredMoldsBySlot]);
    ...
    const handleClearForm = useCallback(() => {
        ...
        setOperatorEmpId(null);
        setBatteryTypes(allBatteryTypes);
        setDetectedBatteryNames(null); // ← reset filter on clear
        getOperators().then(setOperatorList).catch(() => { });
        setHierarchyIds({});
        setHierarchyNames({});
                                        ...
                                        <tr>
                                            ...
                                            {Array.from({ length: slotCount }, (_, slotIdx) => (
                                                <th colSpan={2} key={`type_${slotIdx}`} className="header-type">
                                                    ...
                                                    <SelectBox
                                                        items={
                                                            detectedBatteryNames
                                                                ? detectedBatteryNames
                                                                : batteryTypes.map(t => t.name)
                                                        }
